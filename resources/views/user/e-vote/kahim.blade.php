                                ...
                                @foreach ($data as $kahim)
                                    <div class="col-lg-4 col-md-6">
                                        <div class="card text-white text-center"
                                            ...
                                            <div class="card-body">
                                                <div>
                                                    @if (\File::exists(public_path('assets/kandidat/kahim/' . $kahim->foto)))
                                                        <img src="{{ asset('assets/kandidat/kahim/' . $kahim->foto) }}"
                                                ...
                                                <h4>{{ $kahim->user->nrp }}</h4>
                                                <h5>Nomor Urut : {{ $kahim->nomor_urut }}</h5>
                                                <p>Visi : {{ $kahim->visi }}</p>
                                                <div class="row">
                                                    <div class="col-lg-6 col-md-12">
                                                        <button class="btn btn-success btn-lg btn-block btn-vote"
                                                            data-id="{{ $kahim->id }}"
                                                            data-nomor="{{ $kahim->nomor_urut }}">
                                                            Vote
                                                        </button>
                                                    </div>
                                                    <div class="col-lg-6 col-md-12">
    ...
    <script>
        $(document).ready(function() {
            ...
            myModal.addEventListener('shown.coreui.modal', function() {
                ...
                $.ajax(`${window.baseurl}/user/kahim/` + id, {
                    method: 'GET',
                    success: function(response) {
                        let data = response.data
                        $("#foto").attr('src',
                            `${window.baseurl}/assets/kandidat/kahim/${data.foto}`)
            ...
            $(".btn-vote").on("click", function(e) {
                ...
                Swal.fire({
                    title: 'Apakah Yakin?',
                    text: `Apakah Anda yakin ingin memilih calon ketua himpunan dengan nomor urut ${nomorUrut}`,
                    icon: 'warning',
