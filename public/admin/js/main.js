        ...
        datasets: [
            {
                ...
                backgroundColor: coreui.Utils.hexToRgba(
                    coreui.Utils.getStyle("--primary")
                ),
                borderColor: coreui.Utils.getStyle("--primary"),
                data: [
                    165, 180, 70, 69, 77, 57, 125, 165, 172, 91, 173, 138, 155,
                    89, 50, 161, 65, 163, 160, 103, 114, 185, 125, 196, 183, 64,
                    137, 95, 112, 175,
                ],
            },
            {
                ...
                data: [
                    92, 97, 80, 100, 86, 97, 83, 98, 87, 98, 93, 83, 87, 98, 96,
                    84, 91, 97, 88, 86, 94, 86, 95, 91, 98, 91, 92, 80, 83, 82,
                ],
            },
            {
                label: "Alfa",
                backgroundColor: coreui.Utils.hexToRgba(
                    coreui.Utils.getStyle("--danger")
                ),
...
var bpChart = new Chart(document.getElementById("bp-chart"), {
    type: "bar",
    data: {
        labels: [
            "M",
    ...
    options: {
        ...
        title: {
            display: true,
            text: "Grafik Kehadiran Rapat BP",
            fontSize: 20,
        },
        scales: {
            xAxes: [
...
const getMeetingBP = () => {
    $.ajax({
        url: `${window.baseurl}/admin/api-chart/get-meeting-bp`,
        method: "GET",
        dataType: "json", //parse the response data as JSON automatically
        success: function (response) {
            bpChart.data.labels = [];
            ...
            response.map((res) => {
                ...
                alfa.push(res.alfa);
                sakit.push(res.sakit);
                bpChart.data.labels.push(res.label);
            });
            bpChart.data.datasets[0].data = izin;
            bpChart.data.datasets[1].data = hadir;
            bpChart.data.datasets[2].data = alfa;
            bpChart.data.datasets[3].data = sakit;
            bpChart.update();
        },
    });
};
